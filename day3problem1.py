...
def path(wire):
  ...
  wirey = 0
  pathlist = []
  for num in range(len(wire)):
    if wire[num][0] == "R":
      raiseval = wire[num][1:len(wire[num])]
      raisevalint = int(raiseval)
      wirex = wirex + raisevalint
      for val in range(wirex-raisevalint, wirex):
        coords = tuple([val, wirey])
        pathlist.append(coords)
    elif wire[num][0] == "L":
      raiseval = wire[num][1:len(wire[num])]
      raisevalint = int(raiseval)
      wirex = wirex - raisevalint
      for val in range(wirex, wirex+raisevalint):
        coords = tuple([val, wirey])
        pathlist.append(coords)
    elif wire[num][0] == "U":
      raiseval = wire[num][1:len(wire[num])]
      raisevalint = int(raiseval)
      wirey = wirey + raisevalint
      for val in range(wirey-raisevalint, wirey):
        coords = tuple([wirex, val])
        pathlist.append(coords)
    elif wire[num][0] == "D":
      raiseval = wire[num][1:len(wire[num])]
      raisevalint = int(raiseval)
      wirey = wirey - raisevalint
      for val in range(wirey, wirey+raisevalint):
        coords = tuple([wirex, val])
        pathlist.append(coords)
  return pathlist
